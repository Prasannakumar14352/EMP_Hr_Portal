...
import { Save, MapPin, User as UserIcon, Mail, Phone, Briefcase, Camera, Calendar, AlertTriangle, Lock, ArrowLeft, Building2, Layers, CheckCircle2 } from 'lucide-react';
...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    // Logic update: Allow viewing any profile if ID is provided
    if (id) {
    ...
    if (targetUser) {
      ...
      setFormData({
        ...
        projectIds: targetUser.projectIds || []
      });
      if (targetUser.location) {
        setLocation({ lat: targetUser.location.latitude, lng: targetUser.location.longitude });
      }
    }
  }, [id, currentUser, users, navigate]);
       ...
       <div className="flex flex-col space-y-2">
          <div>
            <button 
              ...
              className="flex items-center text-gray-500 hover:text-emerald-600 transition mb-2"
            >
          ...
          <div className="flex justify-between items-center">
            <div>
              <h2 className="text-2xl font-bold text-gray-800">
                 {isSelf ? 'My Profile' : `${profileUser.name}'s Profile`}
              </h2>
