...
const Organization = () => {
  ...
  const finalizeAllocation = () => {
    if (selectedUser) {
       ...
       notify(`Assignment updated. Email sent to ${selectedUser.name}. System notifications sent to ${colleagueIds.size} colleagues in ${deptName} and related projects.`);
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-in fade-in duration-200">
           <div className="bg-white rounded-xl shadow-2xl p-6 max-w-sm w-full">
              ...
              <p className="text-gray-600 mb-6">
                 Are you sure you want to assign <strong>{selectedUser?.name}</strong> to the selected department and {allocForm.projectIds.length} projects?
                      ...
                      <div className="flex items-center justify-between mt-auto">
                        <div className="flex -space-x-2 overflow-hidden">
                           {users.filter(u => u.projectIds?.includes(proj.id)).slice(0, 3).map(u => (
                             <img key={u.id} className="inline-block h-6 w-6 rounded-full ring-2 ring-white" src={u.avatar} alt={u.name} title={u.name} />
                           ))}
       ...
       {showAllocModal && selectedUser && (
         <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
               ...
               <p className="text-sm text-gray-500 mb-4">Assign department and projects for <span className="font-semibold text-gray-800">{selectedUser.name}</span>.</p>
                        ...
                        {projects.map(proj => (
                           <label key={proj.id} className={`flex items-center space-x-3 p-2 rounded cursor-pointer hover:bg-gray-50 ${allocForm.projectIds.includes(proj.id) ? 'bg-emerald-50 border border-emerald-100' : ''}`}>
                              ...
                              <div className="flex-1">
                                 ...
                                 <span className="text-[10px] text-gray-400 block">{proj.status}</span>
                              </div>
                           </label>
                        ))}
                     </div>
